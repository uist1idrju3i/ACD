...
describe("@acd/adapter-kicad", () => {
  ...
  it("renders the golden fixture with official footprints and no routing", async () => {
    const fixture = JSON.parse(
      await readFile(
    ...
    const board = renderGoldenBoard(fixture);
    expect(board).toContain('(footprint "ESP32-WROOM-32E"');
    expect(board).not.toContain("(segment ");
    expect(renderSchematic(fixture)).toContain('(lib_id "RF_Module:ESP32-WROOM-32E")');
  });
