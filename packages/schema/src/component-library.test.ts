...
describe("component library coverage", () => {
  it("covers both Phase 1 fixture mappings with pinned snapshot footprints", async () => {
    const [golden, prototype2, records, manifest] = await Promise.all([
      readJson<Fixture>("fixtures/phase1/golden-esp32.json"),
      readJson<Fixture>("fixtures/phase1/prototype-2.json"),
      readJson<ComponentLibrary>("fixtures/phase3/component-library.json"),
      readJson<Manifest>("packages/adapters/kicad/library-snapshot/manifest.json"),
    ]);
