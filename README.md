...
| **1 トレーサーバレット**   | 事前変換済み`Requirement` fixture → 部品選定（Phase 1はfixture提供MPN/AVL）→ ネットリスト+BOM → 決定論的な配置／配線（`unrouted=0`必須、goldenは外部routing tool）→ `kicad-cli`検証 → Gerber出力 → **人手で**JLCPCB等へ発注。対象は2層・部品数十点・既製モジュール中心。受入はCLI／fixture runnerで行い、viewerは後続のread-only投影へ延期。smoke fixtureに限り、承認済みの決定論的track/via projectionを例外として使える | 一般用途router、自前の汎用ルーター、WASM化、ナレッジベース、FWパッケージ、自動発注、Phase 1の自然言語入力・sourcing API必須化 | smoke fixtureを内部gateとして通過し、「ESP32＋センサー＋LED」級の基板をfixtureから生成・発注し、**実機が動く**            |
