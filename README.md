...
2. **ブラウザ／AI製品**（[Flux.ai](https://www.flux.ai/p)、[CELUS](https://www.celus.io/)、[Quilter](https://www.quilter.ai/)）は、インストール不要のAI支援設計への需要を示しています。2026年8月時点ではエージェント主導が進んでいますが、いずれもプロプライエタリであり、製造結果・実測フィードバックから学習するループが公開されているとは確認できていません。
3. **オープンなエンジンとフォーマット**（KiCadのファイルフォーマットとIPC API、ヘッドレス`kicad-cli`、[freerouting](https://github.com/freerouting/freerouting)（[DeepWiki](https://deepwiki.com/freerouting/freerouting)） + Specctra DSN/SES、Gerber X2 / IPC-2581、fabのDFMポータル、Octopart/Digi-Key/MouserのAPI）は、エージェントがツールとして呼び出せる決定論的な基盤を提供します。実際、KiCadをLLMエージェントから操作するMCPサーバーは急増しています（[追補](#追補kicadのmcpai拡張エコシステム)）。また、商用EDA大手も制約下の候補探索型のAI（Cadence Allegro X AI等）を投入しています（[追補2](#追補2商用edaのai研究シミュレーションコラボレーション)）。したがって、既存ツールが人間中心のワークフローを補助するだけという整理は、2026年8月時点では限定して読む必要があります。

**既存ツールが埋められていないギャップ：** (a) 対話を明示的で検証可能な要件に変換し、(b) 人間が回路図を描かなくても設計でき、(c) 決定論的なチェッカー（ERC/DRC/シミュレーション/DFM）を自らの提案に対するゲートとして扱い、(d) プロジェクトをまたいで工場からのフィードバックと試作結果から学習する、永続的な設計エージェントです。2026年8月時点では、要件→回路→配線→fabデータまでを自動化する公開実装やKiCad IPCベースのエージェントが現れていますが、実際の製造結果・実測を取り込んで次の設計へ学習反映する公開実装は確認できていません。ACDはこの限定されたギャップを対象とします。
...
_出力：_ 機械可読な要件 + 制約。 _ゲート：_ 要件の整合性チェック（ユーザーによる承認はオプション）。
...
_出力：_ 検証済みネットリスト + BOM + 設計根拠。 _ゲート：_ 自動ERC／整合性チェックに合格する（部品選択と根拠のユーザー承認はオプション）。
...
_出力：_ DRCクリーンな基板レイアウト。 _ゲート：_ DRC合格（人間によるアートワークレビュー・承認はオプション）。
...
_出力：_ fabパッケージ + ファームウェアパッケージ（+ 任意の生成回路図）。
...
_出力：_ 改訂版fabパッケージ + 新しい知識エントリ。 _ゲート：_ fabの受け入れ。

### ステップ6 — 試作立ち上げと改訂

ACDは、設計グラフから生成した**実機テスト項目と観点**によって基板の立ち上げを支援します。テスト計画（電源レールの確認順序、期待電圧、インターフェースのスモークテスト）には、上流の設計根拠から導かれる項目 — たとえばステップ2で付けた「この容量は要チューニング」というチューニング注記や、設計時に許容した既知のリスク — が自動的に含まれ、各項目には「何を」「なぜ」確認するのかという観点が付記されます。測定結果を記録し、問題が発生した際には、残された設計根拠を手がかりに設計内の候補根本原因まで遡って診断・フィードバックできます。設計改訂は、完全な変更追跡とともにステップ2〜4へ戻されます。初回成功、必要な手直し、部品故障などの結果を知識として記録します。将来的には、[Wokwi](https://wokwi.com/)のようなブラウザ内の仮想実機シミュレーションと連携し、実基板が届く前にファームウェアと周辺回路の動作を仮想的に確認することも構想しています。WokwiにはCI（CLI／GitHub Actions）経路と[実験的なMCPサーバー](https://docs.wokwi.com/wokwi-ci/mcp-support)がありますが、仮想試験は実測の代替ではありません。

_出力：_ 実機テスト項目・観点リスト + 検証済み設計改訂版 + 立ち上げレポート。
...
先行事例から、これは実現可能です。[KiCanvas](https://github.com/theacodes/kicanvas)（[DeepWiki](https://deepwiki.com/theacodes/kicanvas)）はKiCadファイルをブラウザ内でレンダリングし、2026年8月時点でも開発が継続していますが、公式にはearly alpha扱いです。tscircuitはブラウザのプレイグラウンドで回路を構築・レンダリングします。また、[OpenBlink Web IDE](https://github.com/OpenBlink/openblink-webide)（[DeepWiki](https://deepwiki.com/OpenBlink/openblink-webide)）がmrubyコンパイラをWASMとして実行するように、複雑なネイティブツールチェーンもWebAssemblyによってブラウザ内で実行できます。
...
- **交換可能なサービスとしての配線：** DSN/SES型の境界を設け、決定論的なルーター（freeroutingなど）と学習型ルーターを交換可能にします。Freerouting v2.3.0（2026年8月7日）にはMCP/A2Aサーバーと実験的なKiCad JSON/API経路が追加されましたが、DSNが公式に本番推奨経路で、KiCad protobuf IPCへの移行はv2.4以降の予定です。ACDは引き続きDSN/SES境界を維持し、必ず独自のDRCを続けて実行します。
- **KiCadエコシステムとの接続：** 検証・エクスポートの二重チェックにはヘッドレス`kicad-cli`（ERC/DRC/Gerber・drill・IPC-2581・ODB++・STEP等の出力、jobset実行）を利用できます。最低対応バージョンはKiCad 10/11を候補としてADR-0007で未決定のまま扱い、Phase 0/1では別途固定したCIプロファイルを使います。ユーザーがKiCadを開いている場合のIPC API連携も、対応バージョンの能力検出を通じて視野に入れます。逆に、ACD自身の操作もMCPで公開し、外部のコーディングエージェントからACDをツールとして呼べるようにします。
...
| [Flux.ai](https://www.flux.ai/p)（[2026 steerable agent](https://www.flux.ai/p/blog/new-steerable-hardware-agent)）                                                                                                 | AIコパイロット／エージェントを備えたプロプライエタリなブラウザECAD。2026年には、実行中に方向修正できるsteerable hardware agentを発表し、春の更新では実行中にERC/DRCを継続実行           | 最も近いUXの先行事例。途中で要件を取り込み、ERC/DRCを実行しながら進むタスク指向エージェントが、単発の生成より重要                                                                                |
| [Quilter](https://www.quilter.ai/)                                                                                                                                                                                  | AIによる物理駆動の自動レイアウト。2026年にはBGAファンアウト自動化beta、Simbeorベースのインピーダンス計算、候補レビューアを発表。現状はスルーホールビアのみで、blind/buried viaは開発中  | レイアウトAIには、多目的スコアカードとハードルールの保証が必要。「見栄えのよい」答えをひとつ出すだけでは不十分                                                                                   |
...
| [EasyEDA](https://easyeda.com/)                                                                                                                                                                                     | JLCPCB/LCSCと連携したブラウザEDA。EasyEDA Proには製造データ／注文APIのbetaがあるが、対話前提でヘッドレス発注APIとは確認できない                                                         | Webからfabまでの摩擦をほぼゼロにできる。ベンダーロックインは避ける                                                                                                                               |
| [SnapMagic Design](https://design.snapmagic.com/)                                                                                                                                                                   | prompt→回路図生成、実コンポーネント、設計チェックを提供するAI製品                                                                                                                       | 回路図・部品選定の自動化と、フットプリント／3Dモデルの出所を分けて検証する                                                                                                                       |
| [Circuit Mind](https://www.circuitmind.io/)                                                                                                                                                                         | 要件／アーキテクチャ→回路図・BOM・検証までを扱うACE。公開製品説明にはPCBレイアウトは含まれない                                                                                          | 要件から回路実装候補を作る工程と、配置・配線・製造工程を分離して検証する                                                                                                                         |
...
| fab DFM（[JLCPCB](https://jlcpcb.com/capabilities/pcb-capabilities)、[PCBWay](https://www.pcbway.com/capabilities.html)）                                                                                           | 自動の製造性チェックと人間からのフィードバック                                                                                                                                          | 2026年8月時点の公開情報で確認できる範囲では、この学習ループを閉じている公開ツールはない。fabデータ生成の自動化があっても、実際の製造結果・実測を取り込んで次の設計へ学習反映することとは別である |
| ソーシングAPI（[Octopart/Nexar](https://www.nexar.com/)、[Digi-Key](https://developer.digikey.com/)、[Mouser](https://www.mouser.com/api-hub/)）                                                                    | NexarはGraphQL＋OAuth2（アクセストークン24時間、無料Evaluationはマッチ部品100点まで）、Digi-KeyはProduct Information API V4＋OAuth2。Mouserの2026年の変更は公開情報では確認できていない | 認証・クォータ・プラン上限を記録し、価格／在庫スナップショットの安定性を保証しない。ソーシングは根拠のある情報検索であり、在庫やピン配置を決して幻覚しない                                       |

### 追補：KiCadのMCP／AI拡張エコシステム

KiCad周辺では、LLMエージェントからKiCadを操作するMCPサーバーが急速に増えています。2026年8月時点の代表例：[mixelpixx/KiCAD-MCP-Server](https://github.com/mixelpixx/KiCAD-MCP-Server)（[DeepWiki](https://deepwiki.com/mixelpixx/KiCAD-MCP-Server)・v2.4.1/v2.5.0（2026年7月）、IPC API + `kicad-cli` + パーサのハイブリッド構成、部品ソーシングまで包含）、[lamaalrajih/kicad-mcp](https://github.com/lamaalrajih/kicad-mcp)（[DeepWiki](https://deepwiki.com/lamaalrajih/kicad-mcp)・リソース／ツール／プロンプトを分離した参照実装）、[Seeed-Studio/kicad-mcp-server](https://github.com/Seeed-Studio/kicad-mcp-server)（[DeepWiki](https://deepwiki.com/Seeed-Studio/kicad-mcp-server)・KiCad 9/10対応、ネット解析からデバイスツリー／テストコード生成）、[eda-agent](https://github.com/salitronic/eda-agent)（[DeepWiki](https://deepwiki.com/salitronic/eda-agent)・Altium/KiCad両対応のレビューエージェント、300超のツール）、[KiPilot](https://github.com/belaszalontai/kipilot-mcp)（[DeepWiki](https://deepwiki.com/belaszalontai/kipilot-mcp)・ガード付き変更に絞ったIPCファースト設計、公式`kicad-python` IPCバインディング経由）、[Circuitron](https://github.com/Shaurya-Sethi/circuitron)（[DeepWiki](https://deepwiki.com/Shaurya-Sethi/circuitron)・SKiDL RAGによる要件→回路生成パイプライン）など。

KiCadは[2026年3月に10.0](https://github.com/KiCad/kicad-source-mirror/releases/tag/10.0.0)がリリースされ、10系の最新patchは10.0.5（2026年7月22日）です。9.xは積極的メンテナンス対象外となりました。公式ソースでは[2026年3月にSWIG、wxPython、旧Python統合が削除](https://github.com/KiCad/kicad-source-mirror/commit/65a442b1d2bf153be7979de8926ab71fd095f4bd)され、IPC APIが今後の経路と明記されています。次期メジャー（KiCad 11、時期未確定）では、master/nightlyに`kicad-cli api-server`（`--socket`）と`kicad-cli gerber`（`gerber convert png`／`gerber diff`／`gerber info`、JSON出力・`--exit-code-only`を含む）、IPC API経由の出力ジョブ、Diff/Merge、pin-to-padマッピング、新インポータが予定されています。これらは開発版由来で変更の可能性があります。`kicad-python`の[公式ドキュメント](https://docs.kicad.org/kicad-python-main/kicad.html)では`get_schematic()`がKiCad 11追加として記載されており、回路図IPCはKiCadのバージョン別能力として扱う必要があります。土台としては、Protocol Buffers + NNGの公式**IPC API**、ヘッドレスな`kicad-cli`（ERC/DRC/各種エクスポート）、Sエクスプレッション操作ライブラリ（[kiutils](https://github.com/mvnmgrx/kiutils)（[DeepWiki](https://deepwiki.com/mvnmgrx/kiutils)）、[kicad-skip](https://github.com/psychogenic/kicad-skip)（[DeepWiki](https://deepwiki.com/psychogenic/kicad-skip)））が使われています。ACDでは将来、CI内IPC利用と製造データ差分検査の候補として評価しますが、最低対応KiCadバージョンは未決定（ADR-0007で決定予定）です。
...
2026年の新しい代表例として、[Konnect](https://github.com/mixelpixx/Konnect)（[DeepWiki](https://deepwiki.com/mixelpixx/Konnect)・KiCad 10向けRustネイティブIPCプラグイン、回路図／PCB編集、ERC/DRC、製造出力、Beta）、[KiCad MCP Pro](https://github.com/oaslananka/kicad-mcp)（[DeepWiki](https://deepwiki.com/oaslananka/kicad-mcp)・read-only既定プロファイル、progressive disclosure、ガード付きwrite/releaseモード）、[mcp-server-kicad](https://github.com/ProductOfAmerica/mcp-server-kicad)（[DeepWiki](https://deepwiki.com/ProductOfAmerica/mcp-server-kicad)・回路図／PCB／シンボル／フットプリント別の操作）、[KiCad AI Assistant](https://github.com/paul356/KiCad-AI-Assistant)（[DeepWiki](https://deepwiki.com/paul356/KiCad-AI-Assistant)・KiCad 10向けLLMチャット＋MCP）、[DCENT_Konduit](https://d-central.tech/dcent-konduit/)（2026年7月31日発表、570超のツールを掲げるOSS MCPエンジン）があります。いずれも新しく、ツール数は成熟度や製造サインオフの権威を意味しません。

### 追補2：商用EDAのAI・研究・シミュレーション・コラボレーション

| 分野                             | 代表例                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ACDへの教訓                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 商用EDAのAI                      | [Cadence Allegro X AI](https://www.cadence.com/en_US/home/tools/pcb-design-and-analysis/pcb-design/allegro-x-ai.html)、[Cerebrus](https://www.cadence.com/en_US/home/products/digital-design-and-signoff/ai-driven-digital-design/cerebrus-intelligent-chip-explorer.html)、[Synopsys DSO.ai](https://www.synopsys.com/ai/ai-powered-eda.html)、[Altium 365](https://www.altium.com/altium-365)のAI機能、[Altium Requirements PortalのAgentic Requirements Engineering](https://resources.altium.com/p/introducing-agentic-requirements-engineering)（2026年7月28日発表） | 要求の作成・変更・レビューとAIスキル、制約下の候補探索が進む。Altiumの常駐バックグラウンドエージェントはcoming soonであり、Synopsysのagentic AI発表は主にチップ設計向け。ACDも多目的スコアと候補履歴を可視化し、不透明な「AIレイアウト」を1つだけ出すことはしない                                                                                                                                                                                                                     |
...
| ブラウザシミュレーション         | [CircuitLab](https://www.circuitlab.com/)、[Wokwi](https://wokwi.com/)、[Falstad/CircuitJS](https://falstad.com/circuit/)、[EveryCircuit](https://everycircuit.com/)、[CRUMB](https://www.crumbsim.com/)                                                                                                                                                                                                                                                                                                                                                                  | ブラウザでの対話的シミュレーションはUXとして実証済み。WokwiはCLI／GitHub ActionsによるCIと実験的MCPサーバーも提供する。特にMCU＋仮想ペリフェラルシミュレーションは、ステップ6の前段として「実基板が届く前にファームウェアを仮想実機でテストする」道を示すが、実測の代替ではない                                                                                                                                                                                                       |
| シミュレーション／検証スタック   | [ngspice](https://ngspice.sourceforge.io/)（WASM／共有ライブラリ候補、上流最新は2026年8月時点で46系（2026年3月））、[Xyce](https://xyce.sandia.gov/)（大規模並列・OSS）、[LTspice](https://www.analog.com/en/resources/design-tools-and-calculators/ltspice-simulator.html)、[QSPICE](https://www.qorvo.com/design-hub/calculators-simulation/qspice)、[PySpice](https://github.com/PySpice-org/PySpice)（[DeepWiki](https://deepwiki.com/PySpice-org/PySpice)）、[IBIS](https://ibis.org/)、オープンソースFEM（[Elmer](https://www.elmerfem.org/)等）                    | ブラウザ内は小～中規模ngspice WASM、ワーカーはngspice共有ライブラリ／Xyce、LTspice／QSPICEは利用者インストールの任意外部検証器として層別する。KiCadのngspice統合もレビュー経路として使い、エンジン版・SPICE方言・モデル出所・収束状態を記録する。ACDのCI固定版は別記し、上流最新版と混同しない。多段階の忠実度（トポロジーチェック→軽量ブラウザシミュレーション→詳細SPICE→実測）を使い分け、モデルの忠実度と不確実性を常に明示する。構造化された測定値とpass/failをエージェントに返す |
| SI／PI・EMC／組込み検証          | [openEMS](https://openems.de/)、[scikit-rf](https://scikit-rf.org/)、[Renode](https://renode.io/)、[QEMU](https://www.qemu.org/)、[IBIS](https://ibis.org/)／[Touchstone 2.1](https://ibis.org/touchstone_ver2.1/touchstone_ver2_1.pdf)                                                                                                                                                                                                                                                                                                                                   | オープンで自動化可能な電磁界・Sパラメータ・組込み仮想実機をワーカー候補とし、商用SI/PI・熱解析ツールは外部Evidenceの比較対象として扱う。モデル形式、ポート、周波数範囲、エンジン版、入力ハッシュを記録し、実測へつなぐ                                                                                                                                                                                                                                                                |
...
| 機械CAD AI／MCP                  | [FreeCAD MCP](https://github.com/neka-nat/freecad-mcp)（[DeepWiki](https://deepwiki.com/neka-nat/freecad-mcp)）、[Fusion MCP](https://github.com/AutodeskFusion360/FusionMCPSample)（[DeepWiki](https://deepwiki.com/AutodeskFusion360/FusionMCPSample)）、[Zoo/KittyCAD](https://zoo.dev/docs/developer-tools/api/ml)、[CADAM](https://github.com/Adam-CAD/CADAM)（[DeepWiki](https://deepwiki.com/Adam-CAD/CADAM)）、[PartCAD](https://github.com/partcad/partcad)（[DeepWiki](https://deepwiki.com/partcad/partcad)）                                                  | MCADにもAI操作境界が出現している。Zoo/KittyCADのText-to-CAD RESTは既存クライアント向けで、新規統合にはML copilot websocketが推奨される。ACDはFreeCAD/Fusion等を任意の型付きMCPピアとして扱い、生成形状はカーネルの妥当性・干渉・寸法ゲートを通す                                                                                                                                                                                                                                      |
| prompt→PCBベンチマーク           | [HWE-Bench](https://doi.org/10.48550/arxiv.2603.18102)、[pcbGPT](https://arxiv.org/html/2606.01188v1)、[PCB-Bench](https://github.com/digailab/PCB-Bench)（[DeepWiki](https://deepwiki.com/digailab/PCB-Bench)）                                                                                                                                                                                                                                                                                                                                                          | ここでのHWE-Benchは基板回路図生成のarXiv 2603.18102を指す（同名のハードウェアバグ修正arXiv 2604.14709とRISC-V CPU設計のhwebench.comは別物）。最高総合pass rateは8.15%、pcbGPTはpass@1 0.90／pass@5 1.00、hardタスク0.72だが、著者自身が専門家レビューの置換には不十分としており、製造・実測まで端から端まで通した評価はなお未成熟                                                                                                                                                     |
...
2026年の新しいOSS例として、[Fragua](https://github.com/mentasystems/fragua)はエージェント駆動で回路図→配置→配線→ERC/DRC→製造DRC→JLCPCB/PCBWay向けfabパッケージまでを扱い、発注は人間がクリックする構成を掲げています。[kicad-agentic-studio](https://github.com/dionjerry/kicad-agentic-studio)等もKiCad向けの要件駆動エージェントを試みています。fabデータ生成までは公開実装で到達しつつありますが、製造結果の取り込みと学習は確認できていません。
...
| **1 トレーサーバレット（完了。実機Evidenceのみ後日）** | 事前変換済み`Requirement` fixture → 部品選定（Phase 1はfixture提供MPN/AVL）→ ネットリスト+BOM → 決定論的な配置／配線（`unrouted=0`必須、goldenは外部routing tool）→ `kicad-cli`検証 → Gerber出力 → **人手で**JLCPCB等へ発注。対象は2層・部品数十点・既製モジュール中心。受入はCLI／fixture runnerで行い、viewerは後続のread-only投影へ延期。smoke fixtureに限り、承認済みの決定論的track/via projectionを例外として使える | 一般用途router、自前の汎用ルーター、WASM化、ナレッジベース、FWパッケージ、自動発注、Phase 1の自然言語入力・sourcing API必須化 | smoke fixtureを内部gateとして通過し、「ESP32＋センサー＋LED」級の基板をfixtureから生成・発注し、実機Evidenceは後日ユーザーが取得する |
...
| **4 実行基盤とブラウザUX**                             | §9のハーネス本格化：タスク台帳、チェックポイント／再開、イベント履歴、予算・ウォッチドッグ。ブラウザUI（read-only 2Dビューア、差分レビュー、タスク台帳表示）、高速チェックのWASM化                                                                                                                                                                                                                                        | 全エンジンのWASM化、実LLM接続、編集UI、3Dビューア、タブレット対応                                                             | 設計ランを途中でブラウザ強制終了しても、最後のチェックポイントから完走できる                                                         |
| **5 AIオーケストレーションと要件対話**                 | BYOK／セルフホストLLMランタイム、自然言語ヒアリングから構造化`Requirement`文書への変換（曖昧さは質問、仮定は記録、バージョン付与）、実sourcing／データシートAPI、サブエージェントレーン、コンテキスト圧縮、token／money予算の実計測、MCPツール境界                                                                                                                                                                        | 独自モデルの学習、組織横断ナレッジ共有、大規模基板対応                                                                        | 自然言語の要求から、人手のfixture作成なしにPhase 1〜4の全ゲートを通過し、発注前ゲートへ到達する                                      |
| **6 FW連携と仮想実機**                                 | ファームウェアパッケージ（ピン割り当て／ペリフェラル設定／HALスタブ）生成、Wokwi等の仮想実機でのFW事前検証、回路図の逆生成                                                                                                                                                                                                                                                                                                | 独自シミュレータ開発                                                                                                          | 実基板の到着前に、生成FWパッケージ＋仮想実機でファームウェアのスモークテストが通る                                                   |
| **7 自働発注**                                         | 基板fab・部品・実装・送料・税・筐体／機械部品を含む**総発注額**が予算上限内での発注。全検証ゲート合格・総発注額・納期・部品在庫の発注前最終チェックを通過した場合のみ、承認IDなしで実行し、異常があれば止まって人に知らせる                                                                                                                                                                                               | 大規模基板対応                                                                                                                | 要件から発注まで人間の操作ゼロ（総発注額が予算上限内）で完走し、届いた基板が動く                                                     |
| **8 スケールと常設運用**                               | cron型の在庫・ライフサイクル監視、ゴールデンタスク回帰評価の常設、大規模基板・自前配線エンジンの検討。Phase 4から移した3Dビューア、タブレット対応                                                                                                                                                                                                                                                                         | —                                                                                                                             | 複数プロジェクトの並行運用下で、モデル・ツール更新後も回帰評価が合格し続ける                                                         |
| **9 ローカル製造対応**                                 | 卓上製造機（切削／導電性インク印刷／導電性フィラメント）向けプリンタ・材料プロファイルDRCとハイブリッド製造判断（[§8](#8-将来展望--家庭で基板を印刷する時代へ)）。非平面の構造エレクトロニクスは将来方向として扱う                                                                                                                                                                                                        | —                                                                                                                             | 同一の設計グラフからローカル試作版とFR-4版の両方が生成・製造できる                                                                   |
...
- **構造エレクトロニクスへの余地：** Phase 9では、筐体表面・成形品・埋め込み配線を含む非平面回路を将来方向として検討します。現時点では`Layout`を平面PCBに固定せず、材料・工程・測定証拠を制約と根拠として保持できる設計グラフを優先します。
...
| ゴールデンタスクによる回帰評価     | 既知の基板設計タスク一式を保持し、モデル・ツール・スキルを更新するたびに全工程をリプレイして電気的妥当性・製造性・コスト・人間介入回数を採点。τ-benchはτ²/τ³-benchへ発展し、Inspect AIは`inspect_swe`で外部CLIエージェントも扱える                                                                                                                          | SWE-bench、τ-bench系、Inspect AI。いずれかが全てを置き換えたのではなく相補的な参照 |
