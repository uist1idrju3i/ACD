...
| 19   | Fab feedback intake             | 3     | implemented   | golden        | 記録済みfab report fixture、対象設計revision、fab profile                              | source provenance、対象参照、重複統合、構造化結果とderivation hashが一致し、unknownは合格扱いにしない                                 | `verification-failed`で停止し、unknownは検証範囲を広げる                                             |
| 20   | KnowledgeItem lifecycle         | 3     | implemented   | golden        | gate 19 intake result、source event、applicability metadata、approval                  | candidateを生成し、provenance・source event・適用条件を検証したうえでreviewed、adoptedへ決定論的に遷移できることを検証する            | `verification-failed`または`schema-invalid`で停止し、承認不足・不正遷移・stale対象を記録             |
...
| 21   | Library overlay patch           | 3     | implemented   | golden        | adopted KnowledgeItem、公式KiCad snapshot、overlay patch、geometry/reopen/DRC Evidence | 宣言済みruleからpatchを導出し、公式snapshotを変更せず、geometry・reopen・DRCに合格した明示的library revisionをproject-localへ採用する | `verification-failed`または`schema-invalid`で停止し、失敗patchは保持する                             |
| 22   | Knowledge application           | 3     | implemented   | golden        | adopted KnowledgeItems、target design context、library revision、projection artifact   | applicable adopted KnowledgeItems are applied through a recorded correction or declared exemption before projection                   | verification-failedで停止し、未適用知識を含む投影を許可しない                                        |
