...
| 19   | Fab feedback intake             | 3     | implemented   | golden        | 記録済みfab report fixture、対象設計revision、fab profile                              | source provenance、対象参照、重複統合、構造化結果とderivation hashが一致し、unknownは合格扱いにしない                                 | `fab-feedback-unknown`で停止し、fab-feedback.jsonをEvidenceとして保持したうえで下流検証範囲を広げる  |
...
| 22   | Knowledge application           | 3     | implemented   | golden        | adopted KnowledgeItems、target design context、library revision、projection artifact   | 適用可能なadopted KnowledgeItemが、記録済みcorrectionまたは宣言済みexemptionを通じてprojectionの前に適用されている                    | verification-failedで停止し、未適用知識を含む投影を許可しない                                        |
