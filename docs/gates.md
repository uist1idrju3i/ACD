...
| 20   | KnowledgeItem lifecycle         | 3     | implemented   | golden        | gate 19 intake result、source event、applicability metadata、approval                  | candidate生成、遷移・scope承認・改訂・deprecation伝播が決定論的に検証され、却下候補とstale結果が保持される                            | `verification-failed`または`schema-invalid`で停止し、承認不足・不正遷移・stale対象を記録             |
