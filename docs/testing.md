...
ゴールデンタスクは合格の確認だけでなく、意図的に注入した欠陥をゲートが検出できることも確認します。検出できなかった欠陥はテストの失敗ではなくゲート規則の欠落として扱い、規則を直してから再実行します。運用中に見逃しが判明した不具合は、対応するfixtureをゴールデンタスクへ追加してから修正を閉じます。

## 仮想実機と実機

Gate 13のschema sampleはsimulated/pending状態であり、schema validationに成功しても
Gate 13 runnerは合格を返しません。実機に依存しないunit testは契約と拒否動作だけを
検証し、測定値の代替にはしません。
