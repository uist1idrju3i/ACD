...
Phase 4の限定決定は、worker-owned JSONLとread-only再接続をADR-0024、Rust／nm整数
固定小数点WASMをADR-0030、Vite＋素TypeScript＋Canvas2D＋HTTP/SSEをADR-0031で定める。
これらは本ADRが保留するプロジェクト全体の最終言語・最終ストレージ決定を置き換えない。
...
実装言語、フレームワーク、スキーマの実行時バリデータ、イベントログ形式、同期・競合解決、
暗号化、マルチユーザー認証、成果物の物理配置はプロジェクト全体では最終的に未決定である。
Phase 4限定のworker／WASM／browser UI決定はADR-0024、ADR-0030、ADR-0031を参照する。
Phase 0の暫定値はADR-0008で参照する。推測で最終決定とせず、決定時に本ADRを後続ADRで
置換する。
