...
- `Last-Event-ID: N`がquery cursorより優先され、受信済みN件の次であるN+1から再開する。
  `?from=N`はN件をskipする意味を維持する。cursorがlog長を超える場合、またはlogの
...
- worker停止とSSE transport切断は別状態として表示し、切断中もworker停止を推定しない。
