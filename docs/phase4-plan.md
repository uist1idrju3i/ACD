...
以下はWP着手前に決める必要があります。推奨案を併記しますが、決定はADRで記録します。

| #        | 決定事項                         | 推奨案                                                                                                                                                                                                       | 影響          |
| -------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- |
| ADR-0024 | 長時間ランの所有者と永続化形式   | ランはworker processが所有し、台帳・checkpointはappend-only JSONL（`.acd/runs/<runId>/`）で保持する。ブラウザは再接続して状態を見る端末に限定し、IndexedDB／OPFSはPhase 4では採用しない                      | WP1、WP2、WP6 |
| ADR-0025 | checkpointの粒度と無効化条件     | checkpointはgate境界単位。input hash、tool／container version、library revision、fab profile、参照KnowledgeItem statusのいずれかが変わったcheckpointはstaleとして再実行する（再利用しない）                  | WP2、WP3      |
| ADR-0026 | 高速チェックのWASM対象と実装言語 | 対象は幾何系の高速チェック（pad間クリアランス、mask sliver、courtyard重なり）に限定。native TS実装を正とし、WASMは同一入力で同一結果を返すparityテスト付きの高速経路として追加する。言語は未決定（Rust推奨） | WP7           |
| ADR-0027 | ブラウザUIの技術選択と範囲       | read-onlyの投影ビューア（2D）、リビジョン差分レビュー、タスク台帳／停止理由の表示までをPhase 4の受入対象とし、編集UI、3Dビューア、タブレット最適化は最小限に留める                                           | WP6           |

「ブラウザ強制終了」の測定方法も決める必要があります（本書§WP3の「未決定」を参照）。
...
- `scripts/phase4-resume.mts`を追加し、Phase 1〜3のgateパイプラインを台帳＋checkpoint上で
  実行する耐久runとして回す。
- 決定論的なstage境界でランの所有processを強制終了し、再開して完走させる。
...
- README §7の「ブラウザ強制終了」を、(a) 実ブラウザをPlaywrightで強制終了して測定するか、
  (b) ランを所有するworker processの強制終了で測定するかは未決定です。推奨は(b)を
  受入gateの正とし、(a)はWP6のUI回帰として追加する案です（ランの所有者はブラウザではなく
  workerであるため、完了条件の本質はworker側の耐久性にあります）。
...
| checkpointの誤再利用            | 古い前提の結果を合格として引き継ぐ | 無効化条件をADR-0025で固定し、staleは再実行                        | 実測後にstale判定範囲を再評価  |
| WASMとnativeの結果差            | 高速経路が別の判定を出す           | parityテスト必須、不一致で停止                                     | 差が出た場合はWASM経路を無効化 |
| ブラウザUIが正のデータを作る    | 設計グラフの権威が崩れる           | UIはread-only、表示値はEvidence IDへ紐付け                         | 編集UIは範囲外として明示       |
| 完了条件の測定方法の解釈差      | 完走を主張できない                 | 測定方法をADRとWP3で先に固定                                       | 未決定事項の決定待ち           |
